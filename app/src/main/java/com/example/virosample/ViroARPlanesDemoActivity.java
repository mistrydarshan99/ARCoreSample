...
public class ViroARPlanesDemoActivity extends ViroActivity implements RendererStartListener {
  private static final String TAG = ViroARPlanesDemoActivity.class.getSimpleName();
  /*
  ...
  @Override public void onRendererStart() {
    ...
    fileDownloader.add("cow.obj",
        "https://softwebsolutions.co.in/titan/web/misc/arkit/3dmodels/cow.obj");
    fileDownloader.add("cow.mtl",
        "https://softwebsolutions.co.in/titan/web/misc/arkit/3dmodels/cow.mtl");
    fileDownloader.start(backgroundThreadHandler, new AsyncFileDownloader.CompletionListener() {
      @Override public void onPolyDownloadFinished(AsyncFileDownloader downloader) {
  ...
  private void processDataFiles() {
    ...
    File file = new File(Environment.getExternalStorageDirectory() + "/Models");
    if (!file.exists()) {
      file.mkdir();
    }
    ...
    for (int i = 0; i < fileDownloader.getEntryCount(); i++) {
      ...
      if (entry.fileName.toLowerCase().endsWith(".obj")) {
        // It's the OBJ file.
        File file1 =
            new File(Environment.getExternalStorageDirectory() + "/Models/" + entry.fileName);
        if (!file1.exists()) {
          try {
            file1.createNewFile();
          } catch (IOException e) {
  ...
  private void createDroidAtPosition(Vector position) {
    ...
    final Bitmap bot = ViroHelper.getBitmapFromAsset(this, "rose_texture.jpg");
    final Object3D object3D = new Object3D();
    object3D.setPosition(position);
    object3D.setScale(new Vector(0.0001, 0.0001, 0.0001));

    //AmbientLight ambient = new AmbientLight(Color.CYAN, 1000.0f);
    //object3D.addLight(ambient);

    mScene.getRootNode().addChildNode(object3D);

    // Load the Android model asynchronously.
    object3D.loadModel(
        Uri.fromFile(new File(Environment.getExternalStorageDirectory() + "/Models/cow.obj")),
