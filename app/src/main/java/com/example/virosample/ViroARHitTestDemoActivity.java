 ...
 * <p>
 * This activity builds an ar scene that lets the user place and drag objects in an ARScene. Click
 * on the Viro
 * button to get a dialog of objects to place in the scene. Drag, rotate and scale the object using
 * pinch and rotate
 * gestures.
 */
public class ViroARHitTestDemoActivity extends ViroActivity implements RendererStartListener {
  ...
  private ARHitTestResult result1;
  private ARHitTestResult result2;
    ...
    private void addModelToPosition(Vector position) {
      ...
      object3D.setGesturePinchListener(new GesturePinchListener() {
        @Override public void onPinch(int i, Node node, float scale, PinchState pinchState) {
          if (pinchState == PinchState.PINCH_START) {
            ...
            object3D.setScale(
                new Vector(scaleStart * scale, scaleStart * scale, scaleStart * scale));
          }
        }
      });
      ...
      object3D.loadModel(Uri.parse(mFileName), Object3D.Type.FBX, new AsyncObject3DListener() {
        ...
        @Override public void onObject3DFailed(String s) {
          Toast.makeText(ViroARHitTestDemoActivity.this,
              "An error occured when loading the 3d Object!", Toast.LENGTH_LONG).show();
        }
      });
